/****** Object:  Function [dbo].[__fn_хНомос]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE FUNCTION [dbo].[fn_хНомос](@project varchar(32), @DATA date)
RETURNS TABLE 
AS
RETURN 
(	-- остатки на дату по проекту
	select g.Название ГдеТ, g.Тип ТипГ, h.Название ЧтоТ, h.Тип ТипЧ
	, isnull(h.ВесКгНетто *o.Количество/h.КолБЕИ,0) Нетто
	, isnull(h.ВесКгБрутто*o.Количество/h.КолБЕИ,0) Брутто
	, o.Количество
