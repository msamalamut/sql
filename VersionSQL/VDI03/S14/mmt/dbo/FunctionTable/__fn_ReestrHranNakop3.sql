/****** Object:  Function [dbo].[__fn_ReestrHranNakop3]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE FUNCTION [fn_ReestrHranNakop3](@project varchar(32), @dn datetime, @dk datetime, @LgotPeriod int)
RETURNS TABLE 
AS
RETURN 
(	-- select * from fn_ReestrHranNakop3('Устькут','20180401','20180430',0) where Партия='укш 932' and mtr=17494 order by npp
	...
	with ba as (
		...
		left outer join (
				SELECT			b.Операция, b.Куда ЧТО, sum(h.ВесКгНетто *b.Количество/h.КолБЕИ) Нетто, sum(h.ВесКгБрутто*b.Количество/h.КолБЕИ) Брутто
				FROM			хДвижение		AS a
				INNER JOIN      хДвижениеСтроки AS b ON a.SYS_ID	= b.Операция
			--	INNER JOIN		хОбъекты		AS o ON b.Откуда	= o.SYS_ID
				INNER JOIN		хОбъекты		AS k ON b.куда		= k.SYS_ID
				INNER JOIN		хОбъекты		AS h ON b.Что		= h.SYS_ID
				where			k.Тип='ГРУЗ' and a.SYS_U is null and b.SYS_U is null and b.Количество<>0 and h.КолБЕИ<>0 
...
--	and not (a.тип='выдача'  and o.Тип='тара')
	...
	--	-- остатки
	--	select h.SYS_ID, 999 s, g.Тип ОткудаТип
	...
	union all
		...
		INNER JOIN		хОбъекты		AS k ON b.куда		= k.SYS_ID
		INNER JOIN		хОбъекты		AS h ON b.Что		= h.SYS_ID
		where			o.Тип='ГРУЗ' and a.SYS_U is null and b.SYS_U is null and b.Количество<>0 
...
--					and o.Код=16671
	...
	select m.ПартияНомер Партия, ОткудаТ Склад, p.УслХранения, k.Название Поклажедатель, p.Сегмент, p.ТСномер ТС, m.Название+isnull(' '+m.маркировка,'') Тмц
	, СДаты, ПоДату
	...
	, a.Количество, m.БЕИ, a.Мест, a.Нетто, a.Брутто
	, a.Операция, a.Номер, a.Транспорт, u.Цена
	, a.npp, m.Код mtr--, m.тип ТмцТип
	from (
		...
		from				bz a
		inner join			bz b on a.SYS_ID=b.SYS_ID and a.Откуда=b.куда and a.npp>b.npp
...
--	inner join			хДвижениеСтроки	a on a.Код=m.Код
