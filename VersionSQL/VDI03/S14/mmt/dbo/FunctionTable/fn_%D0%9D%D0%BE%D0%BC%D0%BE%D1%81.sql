/****** Object:  Function [dbo].[fn_Номос]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE FUNCTION [dbo].[fn_Номос](@DATA date)
RETURNS TABLE 
AS
RETURN 
(
	...
	FROM (
			...
			WHERE а.SYS_U Is Null AND б.SYS_U Is Null AND б.Количество<>0 and а.Тип in ('Закупки','Перемещения','Переброски') and а.Дата<=@DATA
