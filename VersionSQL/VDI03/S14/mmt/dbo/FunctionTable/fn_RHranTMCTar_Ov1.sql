/****** Object:  Function [dbo].[fn_RHranTMCTar_Ov1]    Committed by VersionSQL https://www.versionsql.com ******/

CREATE FUNCTION [dbo].[fn_RHranTMCTar_Ov1](@project varchar(32), @dn datetime, @dk datetime, @LgotPeriod int)
RETURNS TABLE 
AS
RETURN 
(	
-- реестр хранения с накоплением, цена1 из верхнего скалада
-- убрано суммирование веса по таре
-- osmor 16.06.2020
	with ba as (
		-- операции
		SELECT h.SYS_ID
		...
		, o.Тип ОткудаТип
		, cast(isnull(h.ВесКгНетто *b.Количество/h.КолБЕИ,0) as decimal(18,0)) Нетто
		, cast(isnull(h.ВесКгБрутто*b.Количество/h.КолБЕИ,0) as decimal(18,0)) Брутто
		, b.Количество, b.Мест
	...
	and a.Дата<=@dk
	-- отсеить то что уже ушло в тару перемещением до даты выдачи
	and  not (a.тип = 'Выдача' and (b.Куда = b.Откуда) and ((b.Что  in(
	SELECT vds.Что 
...
-- osmor 21.06.2020	and b.Куда<>b.Откуда
...
--	and not (a.тип='выдача'  and o.Тип='тара')
	...
	, cast(isnull(o.Нетто ,0) as decimal(18,0)) Нетто
	, cast(isnull(o.Брутто,0) as decimal(18,0)) Брутто
	...
	-- begin1 08.06.2020 добавил еще уровень для отсечения партии УКШ 2160 а/м В 638 ВО
	left outer join fn_хНомос(@project, @dk) o4 on o3.Где= o4.Что --and o3.ТипЧ  = 'Тара'
	-- end1
	where	o.ТипГ in ('Склад','Сектор','Место','Груз')  --,'Тара')
		and  (CASE 
			...
			WHEN (o2.типЧ = 'Груз') AND o2.ТипГ = 'Тара'  THEN 3 -- в контейнере
			WHEN (o.типг = 'Груз')  AND o2.Где IS NULL THEN 2  --
		-- begin2 08.06.2020  добавил еще уровень для отсечения партии УКШ 2160 а/м В 638 ВО
			WHEN (o3.ТипЧ = 'Груз' or o3.ТипЧ = 'Тара' ) and o4.где is null then 2
		-- end2	 
		ELSE 9 END = 9)
...
--------------------------------------------------------------------------------------------------------------------------------------------
	...
	*/
	,a.Нетто Нетто
	,a.Брутто Брутто
	, a.Операция, a.Номер, a.Транспорт, dbo.fn_SkladPrice(a.Откуда, 1) as цена
	, a.npp, m.Код mtr
	, m.тип ТмцТип
	, m.Маркировка
