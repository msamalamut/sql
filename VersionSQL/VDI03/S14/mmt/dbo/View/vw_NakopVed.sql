/****** Object:  View [dbo].[vw_NakopVed]    Committed by VersionSQL https://www.versionsql.com ******/

create view vw_NakopVed
as
-- select Project, count(*) rc from vw_NakopVed group by Project
	with 
...
--		and a.Тип<>'Приемка' 
	...
	os as (
		...
		, cast(SUM(a.ВесКгНетто)	as decimal(18,3)) НеттоО
		, cast(SUM(a.ВесКгБрутто)	as decimal(18,3)) БруттоО
	...
	pt as (
		...
		, m.Название ТМЦ, m.Габариты, m.Маркировка, m.Упаковка, n.Укрупненка
		, m.КолБЕИ, m.БЕИ, m.ВесКгНетто, m.ВесКгБрутто, m.Мест
	
		, a.ТС, a.ТСНомер, a.ТСД, a.ТСДНомер, a.ТСДДата, a.Поставщик, v.Название Поклажедатель, a.КодЮрика, a.Сегмент
		...
		WHERE		--	m.Проект=@project and 
		m.КолБЕИ<>0 and m.Тип in ('товар','груз','тара')
	...
	st as (
			...
			where			a.SYS_U is null and b.SYS_U is null and b.Количество<>0 and a.Тип='Приемка' 
			--and a.Проект=@project
	...
	aa as (
		...
		, m.НомерЗаказа, m.КодМТР, m.Прожект Проект, m.АОФ, m.АОФПричина, m.МХ1, m.ДатаМХ1

		, substring(replace(replace(s.lvl,'-00','.'),'-0','.'),2,200) ПН
		, m.ТМЦ, m.Укрупненка, m.Упаковка, m.Маркировка, m.Габариты, m.Тип, m.БЕИ
	
		-- приемка
		, m.КолБЕИ КолП, m.Мест МестП, m.ВесКгНетто НеттоП, m.ВесКгБрутто БруттоП
		...
		, p.Количество
		, p.Мест
		, cast(m.ВесКгНетто *p.Количество/m.КолБЕИ as decimal(18,3)) Нетто
		, cast(m.ВесКгБрутто*p.Количество/m.КолБЕИ as decimal(18,3)) Брутто
	...
	select ТС, ТСНомер, ТСД, ТСДНомер, ТСДДата, Поставщик, Поклажедатель, КодЮрика, Сегмент
		, НомерЗаказа, КодМТР, Проект, АОФ, АОФПричина
		, МХ1, ДатаМХ1
		, ПН, ТМЦ, Маркировка, Укрупненка, Упаковка, Габариты
